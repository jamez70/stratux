...
function WeatherCtrl($rootScope, $scope, $state, $http, $interval) {
	...
	function setDataItem(obj, data_item) {
        ...
        // If its over an 90 minutes old, mark the background as light red
        data_item.backcolor = "#ffffff";
        if ((dNow > dThen) && (diff_ms > (1000*60*90))) {
            data_item.backcolor = "#ffe4e1";
        }
	}
	...
	function setDataItemWatched(wx, data_item) {
		...
		var diff_ms = Math.abs(dNow - dThen);
		data_item.age = dThen.getTime();
		data_item.time = deltaTimeString(dNow - dThen) + " old";
